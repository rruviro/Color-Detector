...
import com.doctorblue.colordetector.api.RetrofitClient
...
class MainActivity : BaseActivity() {
    ...
    override fun initEvents() {
        ...
        btn_pick_color.setOnClickListener {
            addColor()
            fetchProductData(currentColor.hex)
        }
    ...
    // Fetch product data based on hex value
    private fun fetchProductData(hex: String) {
        RetrofitClient.instance.getProductDetails(hex)
                ...
                override fun onResponse(call: Call<Product>, response: Response<Product>) {
                    if (response.isSuccessful) {
                        ...
                        if (productDetails != null) {
                            ...
                            val itemImage = productDetails.item_image
                            val productId = productDetails.item_id
                            val itemName = productDetails.item_name
                            val brandName = productDetails.brand_name
                            val itemPrice = productDetails.item_price
                            val GL = productDetails.gl
                            val type = productDetails.type
                            val Quantity = productDetails.quantity
                            val itemDate = productDetails.item_date
                            val expirationDate = productDetails.expiration_date
                            val palletId = productDetails.pallet_id

                            product_id.text = productId.toString()
                            product.text = itemName
                            brand_name.text = brandName
                            item_price.text = itemPrice.toString()
                            gl.text = GL
                            quantity.text = Quantity.toString()
                            Typpe.text = type
                            item_date.text = itemDate
                            expiration_date.text = expirationDate
                            pallet_id.text = palletId.toString()
                            // Load the product image into an ImageView
...
//                                .into(binding.productImage)
                            ...
                            Toast.makeText(this@MainActivity, "No product data found", Toast.LENGTH_SHORT).show()
                        }
                    } else {
